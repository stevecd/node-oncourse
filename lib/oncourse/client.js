...
function Client(username, password) {
  ...
  this.urlPaths = {
    ...
    getStandards: "json.axd/LessonPlan/references_linked", //{id: <standards_id>}
      ...
      column.cells.forEach(function(cell) {
        if(cell.standardsId) {
          sequence = sequence.then(function() {
            return qrequest({
              method: 'POST',
              url: that.buildUrl('getStandards'),
              headers: {
                'User-Agent': that.userAgent
              },
              form: {
              ...
              cell.standards = JSON.parse(response[1]);
            });
          });
        }
      });
    ...
    week.columns.forEach(function(column) {
      column.cells.forEach(function(cell) {
        if(cell.hasHomework && cell.date) {
          sequence = sequence.then(function() {
            return that.getLesson(moment(cell.date, "YYYYMMDD"), cell.period).then(function(lesson) {
...
// this specific date and period on oncourse.  Oncourse does keep a revision history on lesson plans
// so you may be able to recover from mistakes.
//
// To do this carefully, one should carefully backup each lesson before posting!
//
// You can do this using getLesson.
//
// ** getLesson is the only way to grab the homework section.
//
